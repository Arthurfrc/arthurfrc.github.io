...
A alteração seria abaixo da linha 21 do programa https://github.com/Arthurfrc/arthurfrc.github.io/blob/main/exercicio_11/labeling.cpp[labeling.cpp], que é onde seria adicionado as linhas de código abaixo para se criar a iamgem com 16 bits para armazenar os rótulos, no final, se coloca uma conversão de volta para 8 bits para poder normalizar a imagem para poder mostrá-la na tela novamente.

[source,cpp]
----
    labelImage = cv::Mat::zeros(height, width, CV_16U);
    [. . .]
    if (image.at<uchar>(i, j) == 255 && labelImage.at<ushort>(i, j) == 0) {
        nobjects++;
        p.x = j;
        p.y = i;
        cv::floodFill(labelImage, p, nobjects);
    [. . .]
